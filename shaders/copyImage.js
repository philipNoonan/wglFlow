...
layout(binding = 0) uniform highp usampler2D uinputMap;
layout(binding = 1) uniform highp sampler2D inputMap;

layout(binding = 0, rgba8ui) writeonly uniform highp uimage2D uoutputMap;
layout(binding = 1, rgba32f) writeonly uniform highp image2D outputMap;
layout(binding = 2, r32f) writeonly uniform highp image2D routputMap;
...
void main()
{
  ...
  if (imageType == 0)
  {
    uvec4 data = texelFetch(uinputMap, pix, 0);
    imageStore(uoutputMap, pix, data);
  }
  else if (imageType == 1)
  {
    vec4 data = texelFetch(inputMap, pix, 0);
